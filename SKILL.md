...
description: Use when building, redesigning, or shipping a Progressive Web App — especially a real-time / multiplayer / notification-driven one on Cloudflare Workers + Durable Objects. Triggers on "build a PWA", "ship a web app", "add push notifications", "make it installable", "add multiplayer", "make it real-time", "add passkey / WebAuthn", "chess app / friends app / turn-based game", "no-scroll landing", "Durable Object app", "add a state machine", "the invitation is stuck", "the notification doesn't fire", "the game screen is scrolling", "iOS Safari layout breaks", "how do I test this on a real phone", "how do we not ship broken things again". Encodes state-machines-first design, a data-seeded visual verification stack (headless + real WebKit + overlap guard), copy discipline that eliminates AI-slop, interaction-feedback taxonomy, agent orchestration discipline (goals-only Codex briefs, one-owner-per-path, monitor-your-own-children), Durable Object hibernation + input-gate correctness, PWA gotchas (per-domain passkeys, notification recovery, iOS install quirks, service worker delivery), accessibility as a state contract, and React↔imperative-animation ownership isolation. Do NOT use for a static marketing page with no server state, or for a purely native mobile app — use frontend-design or platform-specific skills instead.
---
...
2. **Verification is a stack, not a checkbox.** Programmatic tests catch mechanics; a data-seeded visual matrix (headless × viewport × state) catches representations; the iOS Simulator (real WebKit, real browser chrome) catches phone-only regressions headless is structurally blind to; an overlap guard catches states nobody thought to eyeball; a requirements ledger catches regressions on stated invariants. Every layer is a shipping gate. See `references/verification-stack.md`.
...
7. **Run the full visual matrix and OPEN THE CONTACT SHEET.** Look at every cell at zoom. Cross-check the iOS Simulator sheet for mobile cells. Overlap guard must be green.
...
| `references/pwa-specifics.md` | Touching manifest, service worker, passkey / WebAuthn, push permission, HTML head, iOS-specific styles, or origin-aware beacons. Includes origin strategy checklist, domain confusion, handle-probe abuse budget, session lifetime / revocation / deletion, notification-status surface. |
